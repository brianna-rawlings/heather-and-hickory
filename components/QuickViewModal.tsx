...
// Fallback sizes if no Square variations exist
...
export default function QuickViewModal({ product, onClose }: Props) {
  ...
  const handleSelect = (name: string, id?: string) => {
    ...
    setSelectedVariationId(id || null);
  };
      ...
      >
        ...
        <div className="flex-1 p-8 flex flex-col justify-between relative">
          ...
          <div>
            ...
            <p className="text-base font-semibold text-[#435e48] mb-8">
              {selectedVariationId && hasVariations
              ...
              <div className="grid grid-cols-3 gap-2">
                {hasVariations
                  ? product.variations!.map(variation => (
                      <button
                        key={variation.id}
                        onClick={() => handleSelect(variation.name, variation.id)}
